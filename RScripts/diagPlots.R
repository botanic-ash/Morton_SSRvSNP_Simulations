...
diag.plots.gam <- function(model, data=NULL, robust=T) {
  # 2 x 2:  rvf, qqnorm, density resids, fitted vs y
  r <- model$residuals
  f <- model$fitted.values
  par(mfcol=c(2,2))
  plot.continuous(f=f, r=r, f.name="Fitted", r.name="Residuals", 
                  model.class = "gam")
  qqnorm.with.sim.bounds(r, robust=robust, main="Residuals")
  plot(density(r), main="Residuals")
  y <- model$model[,1]
  plot(y~f, xlab="Fitted", ylab=colnames(model$model)[1])
  abline(0, 1, col="Red")
}
...
diag.plots <-
  function(model,
    ...
    if (std) {
      ...
      if (!is.ANOVA |
          force.RvF) {
        plot.continuous(f, r, "Fitted", "Residuals", model.class, model = model)
      }
      #    print("here")
      if (length(r) >= 5000) {
        qqnorm(r)
      } else {
        qqnorm.with.sim.bounds(r)
      }
      if (model.class == "lm") {
        plot(model, which = 4)
      }
    ...
    if (length(col.nos) > 0) {
      for (j in col.nos) {
        if (is.factor(data[, j])) {
          ...
          plot.continuous(data[, j], r, colnames(data)[j],
                          "Residuals", model.class, data = data)
        }
      }
    }
